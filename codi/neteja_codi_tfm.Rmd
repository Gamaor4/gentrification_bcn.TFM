...
```{r}
pmll <- read_csv("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/Taula estadística - Preu mitjà (€) del lloguer d'habitatges.zip")

emn <- read_csv("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/Taula estadística - Edat mitjana de la població segons nacionalitat (Espanya_UE_Resta estranger).zip")

pnbcn <- read_csv("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/Taula estadística - Població per nacionalitat (Espanya_UE_Resta estranger).zip")

epn <- read_csv("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/Taula estadística - Població de 16 anys i més per titulació acadèmica i nacionalitat (Espanya_UE_Resta estranger).zip")

hut <- read_csv("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/Taula estadística - Nombre d’habitatges d’ús turístic.zip")

mapa <- st_read("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/BCN_UNITATS_ADM/0301040100_Barris_UNITATS_ADM.shp")

mapa.d <- st_read("C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/1. Dades_en_brut/BCN_UNITATS_ADM/0301040100_Districtes_UNITATS_ADM.shp")
```


# Dades

## Mapa barcelona (barris i districte)
...
```{r}
boxplot(preu_mitja_lloguer ~ anys, pmll2.b)
```

```{r}
ggplot(pmll2.b, aes(x = anys, y = preu_mitja_lloguer, group = anys)) +
  geom_violin() +
  geom_jitter(aes(color = territori), width = 0.3, size = 1, alpha = 0.5) +
  theme_bw() +
  labs(title="Evolució preu mitjà per any a BCN",
       x="Any",
       y="Preu mitjà") +
...
ggplot(data = pmll2.d, 
  ...
  geom_point()+
  geom_line() +
  geom_line(data = bar, aes(x = anys, y = preu_mitja_lloguer), color = "black") +
...
```{r}
ll.bcn <- pmll2 %>%
  subset(pmll2$tipus_de_territori == "barri" & pmll2$anys == 2023)
...
ggplot(data = llmap$geometry, aes(fill = llmap$preu_mitja_lloguer)) +
  geom_sf() +
...
```{r}
edat.bcn <- emn2 %>%
  subset(emn2$tipus_de_territori == "barri" & emn2$anys == 2023)
...
ggplot(data = emap$geometry, aes(fill = emap$`edat_mitjana.resta del mon`)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()

ggplot(data = emap$geometry, aes(fill = emap$edat_mitjana.espanya)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()

ggplot(data = emap$geometry, aes(fill = emap$`edat_mitjana.resta de la unio europea`)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()

ggplot(data = emap.b$geometry, aes(fill = emap.b$`edat_mitjana.resta de la unio europea`)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()
```
...
ggplot(data = pobmap.b$geometry, aes(fill = pobmap.b$poblacio.espanya)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()

ggplot(data = pobmap.b$geometry, aes(fill = pobmap.b$`poblacio.resta de la unio europea`)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()

ggplot(data = pobmap.b$geometry, aes(fill = pobmap.b$`poblacio.resta del mon`)) +
  geom_sf() +
  scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
  theme_minimal()
```
...
```{r}
a.1 <- ggplot(data = subset(epn3, epn3$tipus_de_territori == "barri"), 
       aes(x = anys, y = `estudis universitaris, cfgs grau superior_resta del mon`, group = territori, color = territori)) +
  geom_point()+
  geom_line() +
  theme(legend.position = "none")
...
```{r}
epn.bcn <- epn3 %>%
  subset(epn3$tipus_de_territori == "barri" & epn3$anys == 2023)

epnmap <- left_join(epn.bcn, bcn_barri, by = "territori")
epnmap.b <- left_join(epn3.b, bcn_barri, by = "territori")

ggplot(data = epnmap$geometry, aes(fill = epnmap$`sense estudis_espanya`)) +
  geom_sf() +
  scale_fill_gradient(low = "#fff7ec", high = "#ffa600") +
  theme_minimal()

ggplot(data = epnmap$geometry, aes(fill = epnmap$`estudis primaris, certificat d'escolaritat, egb_espanya`)) +
  geom_sf() +
  scale_fill_gradient(low = "#fff7ec", high = "#ffa600") +
  theme_minimal()

ggplot(data = epnmap$geometry, aes(fill = epnmap$`estudis universitaris, cfgs grau superior_espanya`)) +
  geom_sf() +
  scale_fill_gradient(low = "#fff7ec", high = "#ffa600") +
  theme_minimal()
```
...
```{r}
hut3 <- hut2 %>%
  subset(hut2$tipus_de_territori == "districte") %>%
  select(-tipus_de_territori) %>%
  pivot_longer(cols = -territori,
               names_to = "anys",
...
```{r}
boxplot(habitatges_us_turistic ~ territori, hut3)

boxplot(habitatges_us_turistic ~ anys, hut3)
```
...
```{r}
habtur.bcn <- hut4 %>%
  subset(hut4$tipus_de_territori == "barri" & hut4$anys == 2023)

habturmap <- left_join(habtur.bcn, bcn_barri, by = "territori")

hutmap <- left_join(hut.b, bcn_barri, by = "territori")

ggplot(data = habturmap$geometry, aes(fill = habturmap$habitatges_us_turistic)) +
  geom_sf() +
  scale_fill_gradient(low = "#ffffff", high = "#ff2424") +
  theme_minimal()
```
...
library(ggplot2)
library(gridExtra)
library(sf) # Ensure you have the sf package loaded

# Calculate the global minimum and maximum across all datasets
...
# hut2023 <- st_read("path_to_2023_data")
# hut2020 <- st_read("path_to_2020_data")
# hut2017 <- st_read("path_to_2017_data")
# hut2014 <- st_read("path_to_2014_data")

create_plot <- function(data, year) {
  min_val <- min(data$habitatges_us_turistic, na.rm = TRUE)
  max_val <- max(data$habitatges_us_turistic, na.rm = TRUE)
  mid_val <- (min_val + max_val) / 2
  
  ggplot() +
    geom_sf(data = data$geometry, aes(fill = data$habitatges_us_turistic)) +
...
h1.b <- create_plot(hut2023, 2023)
h2.b <- create_plot(hut2020, 2020)
h3.b <- create_plot(hut2017, 2017)
h4.b <- create_plot(hut2014, 2014)
...
write_csv(
  pmll2,
  "C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/2. Dades_netes/preu_mitja_lloguer_BCN.csv",
  na = "NA"
)

write_csv(
  emn2,
  "C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/2. Dades_netes/edat_mitjana_nacionalitat_BCN.csv"
)

write_csv(
  pnbcn2,
  "C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/2. Dades_netes/poblacio_nacionalitat_BCN.csv",
  na = "NA"
)

write_csv(
  epn3,
  "C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/2. Dades_netes/estudis_poblacio_nacionalitat_BCN.csv"
)

write_csv(
  hut4,
  "C:/Users/Usuario/OneDrive/Documents/UOC/master_ciencia_dades/TFM/Modul 3/2. Dades_netes/habitatges_us_turistic_BCN.csv",
  na = "NA"
)
...
```{r}
unique(m.df[m.df$tipus_de_territori == "barri" ,"territori"])
```
...
```{r}
unique(m.df[m.df$tipus_de_territori == "districte", "territori"])
```
...
df_plot <- data.frame(umap_result$layout)
colnames(df_plot) <- c("UMAP1", "UMAP2")
...
# Plot using ggplot2
xyn <- ggplot(df_plot, aes(x = UMAP1, y = UMAP2, color = labels)) +
...
df_plot.b <- data.frame(umap_result.b$layout)
colnames(df_plot.b) <- c("UMAP1", "UMAP2")

labels.b <-  as.factor(km_res.b$cluster)

df_plot.b$labels.b <-  labels.b

# Plot using ggplot2
xyn.b <- ggplot(df_plot.b, aes(x = UMAP1, y = UMAP2, color = labels.b)) +
    geom_point(alpha = 0.6) +
    labs(title = "UMAP Projection", x = "UMAP1", y = "UMAP2") +
    theme_minimal()
...
```{r}
scale.01 <- function(x) {
  # Remove NA values for min and max calculations
  x_min <- min(x, na.rm = TRUE)
  x_max <- max(x, na.rm = TRUE)
  return((x - x_min) / (x_max - x_min))
}

m.df.b <- subset(m.df.barri, m.df.barri$territori == "el poblenou") %>%
  mutate(across(!c("territori", "anys"), scale.01)) %>%
  pivot_longer(cols = -c("territori", "anys"), names_to = "variable", values_to = "valor")
```

```{r}
asd <- subset(m.df.barri, m.df.barri$territori == "el poblenou") %>%
  mutate(across(!c("territori", "anys"), scale))
plot(poblacio.espanya ~ preu_mitja_lloguer , asd)
```


```{r}
el_raval <-  ggplot(m.df.b, aes(x = anys, y = valor, color = variable)) +
  geom_point() +
  geom_line() +
...
unique_zones <- unique(m.df.barri$territori)

# Create a list to store plots
plot_list <- list()

for(i in seq_along(unique_zones)) {
  zone_data <- subset(m.df.barri, territori == unique_zones[i]) %>%
  mutate(across(!c("territori", "anys"), scale.01)) %>%
  pivot_longer(cols = -c("territori", "anys"), names_to = "variable", values_to = "valor")
  
  p <- ggplot(zone_data, aes(x = anys, y = valor, color = variable)) +
    geom_point() +
    geom_line() +
    scale_color_manual(values = color_variable) +
    theme(legend.position = "bottom") +
    ggtitle(paste("Plot for Zone:", unique_zones[i]))
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("year", "Year:", min = 1997, max = 2023, value = 1997, step = 1)
    ),
    mainPanel(
        plotOutput("mapPlot", height = "600px", width = "600px")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$mapPlot <- renderPlot({
    # Create a subset of your dataframe based on the selected year
    df_year <- mapbcn.d[mapbcn.d$anys == input$year, ]
    
    p1 <- ggplot() +
      geom_sf(data = df_year$geometry, aes(fill = factor(df_year$cluster))) +
      scale_fill_manual(values = c("1" = "red", "2" = "yellow", "3" = "green")) +
      theme_minimal()
    
    p2 <- ggplot(data = emap$geometry, aes(fill = emap$edat_mitjana.espanya)) +
      geom_sf() +
      scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
      theme_minimal()
    ...
    # Combine the plots
    grid.arrange(p1, p2, ncol = 1, heights = c(1.5, 1.5))
  })
}
...
ui <- fluidPage(
  ...
  sliderInput("year1", "Select Year for Plot 1:", 
              min = 1997, max = 2023, value = 1997, step = 1),
  
  sliderInput("year2", "Select Year for Plot 2:", 
              min = 1997, max = 2023, value = 1997, step = 1),
...
server <- function(input, output, session) {
  ...
  output$mapPlot <- renderPlot({
    # Create a subset of your dataframe based on the selected year
    df_year1 <- mapbcn.d[mapbcn.d$anys == input$year1, ]
    df_year2 <- emap[emap$anys == input$year2, ]
    
    p1 <- ggplot() +
      geom_sf(data = df_year1$geometry, aes(fill = factor(df_year1$cluster))) +
      scale_fill_manual(values = c("1" = "red", "2" = "yellow", "3" = "green")) +
      theme_minimal()
    
    p2 <- ggplot(data = df_year2$geometry, aes(fill = df_year2$edat_mitjana.espanya)) +
      geom_sf() +
      scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
      theme_minimal()
...
# Define the server
server.b <- function(input, output, session) {
  # Plot
  output$mapPlot <- renderPlot({
    # Create a subset of your dataframe based on the selected year
    df_year <- mapbcn[mapbcn$anys == input$year, ]
...
# Run the app
shinyApp(ui = ui.b, server = server.b)
```
...
# Assuming you have a data frame called 'emap' with columns 'anys', 'geometry', and other columns
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("year", "Year:", min = 1997, max = 2023, value = 1997, step = 1),
      selectInput("column_selector", "Select Column:",
...
server <- function(input, output, session) {
  output$mapPlot <- renderPlot({
    # Create a subset of your dataframe based on the selected year
    df_year <- emap[emap$anys == input$year, ]
    
    # Create the map plot using ggplot2
    ggplot(data = df_year$geometry, aes(fill = df_year[[input$column_selector]])) +
      geom_sf() +
      scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
      theme_minimal()
  })
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("year1", "Year:", min = 1997, max = 2023, value = 1997, step = 1),
      selectInput("column_selector", "Select Column:",
                  choices = colnames(select(emap, -c('territori', 'anys', 'NOM', 'geometry'))))
    ),
    mainPanel(
       tabsetPanel(
        ...
        tabPanel("panel 1", "one",
                 plotlyOutput("mapPlot", width = "100%", height = "600px"),
...
# Define the server
server <- function(input, output, session) {
  output$mapPlot <- renderPlotly({  # Use renderPlotly instead of renderPlot
    # Create a subset of your dataframe based on the selected year
    emnly <- emap.b[emap.b$anys == input$year1, ]
    llly <- llmap.b[llmap.b$anys == input$year2, ]
    
    
    # Create the ggplot plot
    e <- ggplot(data = emnly$geometry, aes(fill = emnly[[input$column_selector]], text = paste("Value: ", sprintf("%.2f", emnly[[input$column_selector]]), "<br>Territori: ", emnly[["territori"]]))) +
      geom_sf() +
      scale_fill_gradient(low = "#6ff237", high = "#ba37f2") +
      theme_minimal() +
      labs(fill = "Edat de Barcelona")
    
    # Convert ggplot plot to Plotly
    ggplotly(e, tooltip = "text") 

    l <- ggplot(data = llmap$geometry, aes(fill = llmap$preu_mitja_lloguer)) +
      geom_sf() +
      scale_fill_gradient(low = "green", high = "red") +
      theme_minimal()
  ...
  output$data <- renderDataTable({
    nearPoints(emap, input$plot_click)
  })
}
...
# Assuming your datasets are loaded as emap, llmap, pobmap.b, epnmap.b, hutmap, mapbcn, and mapbcn.d

# Calculate the global minimum and maximum values for relevant datasets
...
server <- function(input, output, session) {
  ...
  output$mapPlot3 <- renderPlotly({
    ...
    # Calculate the dynamic limits for the selected column
  ...
  output$panel3Text <- renderText({
    paste("Content for panel 3")
  })
  
  output$panel4Text <- renderText({
    paste("Content for panel 4")
  })
  
  output$panel5Text <- renderText({
    paste("Content for panel 5")
  })
  
  output$panel6Text <- renderText({
    paste("Content for panel 6")
  })
  
  output$panel7Text <- renderText({
    paste("Content for panel 7")
  })
}
...
```{r}
bb <- data.frame(m.df.barri[,], cluster = km_res.b$cluster) %>%
  mutate(across(!c("territori", "anys", "cluster"), scale))

bb_s <- bb[, !(names(bb) %in% c("territori", "anys"))]
...
bb_predictors <- bb_s[, !(names(bb_s) == "cluster")] %>%
  mutate_all(function(x) {
    if(is.numeric(x)) {
      x[is.na(x)] <- mean(x, na.rm = TRUE)
    }
...
# Apply PCA to the predictor variables
pca <- prcomp(bb_predictors, scale. = TRUE)

# Print summary of the PCA
summary(pca)

# # Use the principal components as predictors in the multinomial logistic regression model
# bb_pca <- data.frame(cluster = bb$cluster, pca$x)
# 
# # Fit the multinomial logistic regression model
# model_pca <- multinom(cluster ~ ., data = bb_pca)
# 
...
```{r}
k12 <- data.frame(pca$rotation[,c(1:3)]) %>%
...
```{r}
kak <- ggplot(kk2, aes(x = PCs, y = valor, group = variables, colour = variables)) + 
...
result <- adonis2(mDF_b ~ km_res.b$cluster, method = "euclidean", permutations = 999)
result_pw <- pairwise.adonis(mDF_b, km_res.b$cluster, perm = 999)
...
tree_model <- rpart(cluster ~ ., data = b3, method = "class")
```


```{r}
# Predicting on the same dataset (for simplicity, usually you should split into train/test)
pred <- predict(tree_model, b3, type = "class")

# Confusion matrix
table(Predicted = pred, Actual = b3$cluster)

# Accuracy
accuracy <- sum(pred == b3$cluster) / nrow(b3)
...
tree_model <- rpart(cluster ~ ., data = df_train, method = "class")
...
df_test$cluster <- factor(df_test$cluster, levels = levels(b3$cluster))

# Confusion matrix
conf_matrix <- confusionMatrix(pred, df_test$cluster)
print(conf_matrix)
...
# Convert predictions to factor with the same levels as the actual cluster values
pred <- factor(pred, levels = levels(df_test$cluster))

# Ensure the 'cluster' column in the test set is a factor with the same levels as the training set
df_test$cluster <- factor(df_test$cluster, levels = levels(df_train$cluster))
